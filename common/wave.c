...
int load_wav(float **signal, int* num_frames, int *num_channels, int* sample_rate, const char* path, FILE *fp){
  ...
  // NOTE: works only on little-endian architecture
  char chunkID[4]; // = {'R', 'I', 'F', 'F'};
  if(fread((void*)chunkID, sizeof(chunkID), 1, fp) != 1)
    goto quit;
  uint32_t chunkSize; // = 4 + (8 + subChunk1Size) + (8 + subChunk2Size);
  if(fread((void*)&chunkSize, sizeof(chunkSize), 1, fp) != 1)
    goto quit;
  char format[4]; // = {'W', 'A', 'V', 'E'};
  if(fread((void*)format, sizeof(format), 1, fp) != 1)
    goto quit;
  if(feof(fp)){
  ...
  while(!feof(fp)){
	...
	goto quit;
      ...
      if(fread((void*)&audioFormat, sizeof(audioFormat), 1, fp) != 1)
	goto quit;
      if(fread((void*)&numChannels, sizeof(numChannels), 1, fp) != 1)
	goto quit;
      *num_channels = numChannels;
      if(fread((void*)&sampleRate, sizeof(sampleRate), 1, fp) != 1)
	goto quit;
      if(fread((void*)&byteRate, sizeof(byteRate), 1, fp) != 1)
	goto quit;
      if(fread((void*)&blockAlign, sizeof(blockAlign), 1, fp) != 1)
	goto quit;
      if(fread((void*)&bitsPerSample, sizeof(bitsPerSample), 1, fp) != 1)
	goto quit;
      if(feof(fp)){
	...
	fseek(fp,chunkSize-16,SEEK_CUR); // Skip unsupported chunk
      }
      if (numChannels != 1){
	...
	*signal = malloc(nbytes);
	...
	fseek(fp,extra,SEEK_CUR);
      ...
      fseek(fp,chunkSize,SEEK_CUR); // Skip unsupported subchunk
    }
  }
  return 0;
 quit:
