...
bool Run_queue = false;  // When true, exit after running queue (suitable for
                         // calling from cron)
...
int process_file(
    char const *path, bool is_ft8,
    double base_freq); // Either file or directory (calls recursively)
void process_directory(char const *path, bool is_ft8,
                       double base_freq); // Directory only; called recursively
...
int main(int argc, char *argv[]) {
  ...
  // Base freq; if not specified with -f in megahertz, extracted from input
  // filename of form yyyymmddThhmmssZ_ffffffff_usb.wav where yyyymmdd is year,
  // month, day hhmmss is hour, minute, second UTC ffffffffff is frequency in
  // *hertz*
  double base_freq = 0;
  int c;
...
#ifdef __linux__ // inotify is linux-only; non-linux will run simple timer-based
  ...
  // Be careful with IN_CREATE; we don't want to read ordinary files until
  // they're closed, but we do want to wach for newly created directorie
  ...
  while (true) {
    ...
    // Will happen on the first loop since last_poll{0} is now in the distant
    // past
    struct timespec now;
    ...
    while ((len = read(fd, buffer, sizeof(buffer))) > 0) {
      char *file_list[SORT_SIZE] = {
          0}; // List of files to sort and process from each event
      int filecount = 0;
      int event_num = 0;
      for (int i = 0; i < len;
           i += sizeof(struct inotify_event) + event->len, event_num++) {
        ...
        // We could find 'wd' with a null path if it had been created and
        // deleted
        ...
        if (Verbose > 1) {
          fprintf(
              stderr,
              "wd %d, event %d, directory %s, name %s, length %d, event 0x%x ",
              event->wd, event_num, dirname, event->name, event->len,
              event->mask);
        ...
        if (strcmp(event->name, ".") == 0 || strcmp(event->name, "..") == 0)
          continue; // igore anything regarding the current directory or its
                    // parent

        if (event->mask &
            IN_IGNORED) { // both flags set when directory is deleted
          ...
          // Remove from hash table BUT leave wd unchanged as a "tombstone" for
          // the deleted entry so searches for other nearby values won't
          // erroneously fail
        ...
        char *fullname = NULL;
        int r = asprintf(&fullname, "%s/%s", dirname,
                         event->name); // mallocs memory for fullname
        if (r <= 0) {
        ...
        case S_IFREG:
          if ((event->mask & (IN_CLOSE_WRITE | IN_MOVED_TO)) &&
              has_suffix(fullname, ".wav")) {
            // The usual sequence is for pcmrecord to write a .wav.tmp file and
            // atomically rename it This causes a IN_CLOSE_WRITE event on the
            // tmp followed by IN_MOVED_TO the wav filename We also monitor
            // IN_CLOSE_WRITE in case a .wav file is written directly, without
            // renaming Add to list for sorting
            if (filecount < SORT_SIZE)
              file_list[filecount++] = strdup(
                  fullname); // mallocs memory, freed after sort and process
            else
              process_file(fullname, is_ft8,
                           base_freq); // sort table is full (unlikely) so just
                                       // process it
          }
          break;
        case S_IFDIR:
      ...
      // Doesn't really work well because the events are often split across
      // several reads and I'd rather not delay artificially
  ...
  while (true) {
    ...
    if (Run_queue)
      break;
    sleep(1 + (random() & 7)); // Random sleep between 1 and 8 sec; prevent
                               // synchronizing of multiple workers
  }
#endif
  exit(0);
}
...
int add_watches_recursive(int const fd, const char *path) {
  ...
  int const wd = inotify_add_watch(fd, path,
                                   IN_CLOSE_WRITE | IN_MOVED_TO |
                                       IN_DELETE_SELF | IN_CREATE | IN_ISDIR);
  if (wd == -1) {
  ...
  int i;
  for (i = 0; i < HSIZE; i++) {
    if (Wd_hashtab[hp].wd == wd ||
        Wd_hashtab[hp].wd == 0) // Will be wd if previously used
      break;
    hp = (hp + 1) % HSIZE; // keep scanning
  }
  if (i == HSIZE) {
  ...
  free(
      Wd_hashtab[hp]
          .path); // Just in case it's already present, but how can this happen?
  ...
  while ((ent = readdir(dir)) != NULL) {
    if (ent->d_type == DT_DIR && strcmp(ent->d_name, ".") != 0 &&
      ...
      if (r <= 0) {
        free(subpath);
        fprintf(stderr,
                "add_watches_recursive(%s) failed when adding %s, no memory\n",
                path, ent->d_name);
...
void print_inotify_mask(uint32_t mask) {
  ...
  if (mask & IN_ONLYDIR)
    fprintf(stderr, "IN_ONLYDIR ");
  if (mask & IN_DONT_FOLLOW)
    fprintf(stderr, "IN_DONT_FOLLOW ");
  if (mask & IN_EXCL_UNLINK)
    fprintf(stderr, "IN_EXCL_UNLINK ");
  if (mask & IN_MASK_ADD)
    fprintf(stderr, "IN_MASK_ADD ");
  if (mask & IN_ONESHOT)
    fprintf(stderr, "IN_ONESHOT ");
}
#endif

// Process a single audio file, delete if successful
// Return -1 on decoding error, 0 on success, 1 if the file couldn't be found or
// locked
int process_file(char const *const path, bool is_ft8, double base_freq) {
  ...
  for (; tries >= 0; tries--) {
    ...
    if (rcount != sizeof pid) {
      // rcount == 0 might happen if another task has created the lockfile but
      // not yet written to it however, if the empty lock file is stale, we'll
      // continue to treat the file as locked no truly race-free way to handle
      // this, except by looking at its age
      if (rcount < 0)
    ...
    // If it does, kill will return 0 if we own it, or -1 and errno == EPERM if
    // it exists and owned by someone else
  ...
  // load_wav now allocates signal, we must free (unless we exit right away, as
  // we currently do)
  assert(path != NULL);
  ...
  if (rc < 0 || num_samples < (is_ft8 ? 12.64 : 4.48) * sample_rate) {
    ...
    if (lstat(path, &statbuf) == 0) {
      ...
      if (age > 60) {
        ...
        // Could actually do this after 15 sec or so, but the directory scan
        // will get it eventually
        if (NoDelete) {
          fprintf(stderr, "%s: short/bad file, %'lld bytes, %'d seconds old\n",
                  path, (long long)statbuf.st_size, age);
        } else {
          fprintf(
              stderr,
              "%s: short/bad file, %'lld bytes, %'d seconds old, deleting\n",
              path, (long long)statbuf.st_size, age);
          int r = unlink(path);
          if (r != 0)
  ...
  if (base_freq == 0) {
    assert(path != NULL);
    // Look first for extended file attribute "user.frequency" (linux) or
    // "frequency" (macos)
    char att_buffer[1024] = {0}; // Shouldn't be anywhere near this long
#ifdef __linux__
    ssize_t s =
        getxattr(path, "user.frequency", att_buffer, sizeof(att_buffer) - 1);
#else
    ssize_t s =
        getxattr(path, "frequency", att_buffer, sizeof(att_buffer) - 1, 0, 0);
#endif
    if (s > 0) {
      ...
      if ((cp = strchr(path, '_')) != NULL &&
          (cp1 = strrchr(path, '_')) != NULL) {
  ...
  bool tmp_set = false;
  {
    // Look first for extended file attribute "user.unixstarttime" or
    // "unixstarttime"
    char att_buffer[1024] = {0};
#ifdef __linux__
    ssize_t s = getxattr(path, "user.unixstarttime", att_buffer,
                         sizeof(att_buffer) - 1);
#else
    ssize_t s = getxattr(path, "unixstarttime", att_buffer,
                         sizeof(att_buffer) - 1, 0, 0);
#endif
    if (s > 0) {
      // Extract from attribute
  ...
  if (!tmp_set) {
    ...
    char junk;
    int r = sscanf(bn, "%04d%02d%02d%c%02d%02d%02d", &year, &mon, &day, &junk,
                   &hr, &minute, &sec);
    free(npath);
    if (r == 7) {
  ...
  if (!tmp_set) {
    // That didn't work either, so subtract 7.5 or 15 sec from the modification
    // time not really tested, but seems simple enough
    struct stat statbuf = {0};
    if (lstat(path, &statbuf) == 0) {
      struct timespec ts = {
  ...
  // Done with the file (could have been deleted earlier, but just in case we
  // crash)
  if (!NoDelete) {
  ...
  while ((d = readdir(dirp)) != NULL) {
    ...
    case DT_REG:
      if (has_suffix(d->d_name, ".wav") && filecount < SORT_SIZE)
        file_list[filecount++] =
            strdup(d->d_name); // d_name changes when readdir is called again
      break;
    default:
      break;
    }
